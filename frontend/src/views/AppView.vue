<template>
  <div class="title-container">
    <h1 class="gradient-text">我的应用</h1>
  ...
  <el-dialog v-model="dialogFormVisible" title="新建应用" width="500">
    <el-form :model="form" :rules="rules" ref="formRef">
      <el-form-item label="应用名字" :label-width="formLabelWidth" prop="name">
        <el-input
          v-model="form.name"
          autocomplete="on"
          placeholder="请输入应用名字"
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button type="primary" @click="createNewAPPBaseHandle()">
          确定
        </el-button>
      </div>
    </template>
...
async function fetchDataSet() {
  try {
    const response = await getAppsetsAxios()
...
// 表单验证规则
const rules = {
  name: [{ required: true, message: '请输入应用名字', trigger: 'blur' }],
  description: [
...
// 创建新应用的处理函数
const createNewAPPBaseHandle = () => {
  formRef.value.validate(async (valid) => {
    if (valid) {
      // 如果表单通过验证，则继续执行提交操作
      await postAppsetAxios({
