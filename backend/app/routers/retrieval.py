...
async def delete_file(filename: str):
    ...
    if file_path.exists():
        ...
        return {"message": f"{filename} 删除成功", "filename": filename}
    else:
...
async def get_temp_pdf_files(
    chunk_size: int = Query(default=500, ge=0),
...
async def edit_chunk_by_metadata_id(
    dataset_id: str,
    ...
    if not chunk["ids"]:
        raise HTTPException(
            status_code=404,
            detail=f"Chunk with ID {metadata_id} not found in dataset {dataset_id}",
        )
    # 创建一个新的 Document 实例
...
async def delete_chunk_by_metadata_id(
    dataset_id: str,
