...
class ChatHistoryCreate(BaseModel):
    ...
    chat_id: Optional[str] = Field(None, description="ChatSet的ID")  # 如果没有提供 chat_id，存储到默认的测试 ChatSet
    question: str = Field(..., description="提出的问题")
    answer: str = Field(..., description="对问题的回答")
    cite_documents: Optional[List[CiteDocument]] = Field([], description="检索到的引用记录")
    context_histories: Optional[list[Dict[str, Any]]] = Field([], description="历史上下文记录")
...
async def get_related_chat_ids(
    ...
    chatsets = await ChatSet.filter(app_id=appset, is_test=False).all().order_by('-created_at')
