...
class Article(Model):
    id = fields.UUIDField(pk=True)
    dataset_id = fields.ForeignKeyField(
        "models.DataSet", related_name="articles", on_delete=fields.CASCADE
    )
    name = fields.CharField(max_length=255)
...
class QueryTestHistory(Model):
    id = fields.UUIDField(pk=True)
    dataset_id = fields.ForeignKeyField(
        "models.DataSet", related_name="query_test_histories", on_delete=fields.CASCADE
    )
...
class ChatHistory(Model):
    id = fields.UUIDField(pk=True)
    chat_id = fields.ForeignKeyField(
        "models.ChatSet", related_name="histories", on_delete=fields.CASCADE
    )
