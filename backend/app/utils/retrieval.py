...
def PDF_to_documents(
    ...
    for file_path in file_paths:
        ...
        for document in temp_documents:
            ...
            document.metadata["total_word_count"] = (
                total_word_count  # 添加总字数到metadata
            )
            document.metadata["chunk_word_count"] = len(
                document.page_content
            )  # 添加分块字数到metadata
            document.metadata["document_metadata_id"] = str(
                uuid.uuid4()
            )  # 添加document_id到metadata
            document.metadata["article_id"] = article_id  # 添加article_id到metadata
            document.metadata["dataset_id"] = str(
                dataset_id
            )  # 添加dataset_id到metadata
            count += 1
...
def transform_data(data, keep_keys, new_keys=None):
    ...
    for i in range(data_length):
        entry = {
            new_keys[index]: data[key][i]
            for index, key in enumerate(keep_keys)
            if key in data
        }
...
def retrieval_similarity_search(
    dataset_ids: list[UUID], query: str, k: int, min_relevance: float
):
    print(k)
    if k < 1:
        return []
    ...
    for dataset_id in dataset_ids:
        try:
            ...
            all_docs_and_scores.extend(
                [
                    {**document.__dict__, "score": 1 - score}
                    for document, score in docs_and_scores
                    if 1 - score >= min_relevance
                ]
            )
    ...
    sorted_docs_and_scores = sorted(
        all_docs_and_scores, key=lambda x: x["score"], reverse=True
    )[:k]
